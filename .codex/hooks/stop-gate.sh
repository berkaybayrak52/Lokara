...
# $CODEX_PROJECT_DIR points at the main checkout, so inside a worktree this graded the
...
# whose changes are being graded; CODEX_PROJECT_DIR is only the fallback.
cd "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || cd "${CODEX_PROJECT_DIR:-$PWD}"
