...
- Page 01 is completely transcribed in `docs/08`/`docs/02` with a 24-ID data oracle and green
  ...
  fixtures. Page 03 is completely transcribed, approved and merged in `docs/10`. Page 04 is
  source-backed and prepared on `slice/docs-11`, but is not approved or merged; Pages 06 and 07
  still need D2 transcription before related work.
...
- Phase D2's `docs/15` review gate was approved and its slice merged on **20.08.2026**. The
  ...
  The `docs/10` slice was approved and merged on **21.08.2026**. The `docs/11` slice is prepared
  for Emir's review with no implementation or commit.
...
| Page 04 — Anlage V + DATEV | `docs/11-tax-export.md` | `11-F01…F16` | Prepared on `slice/docs-11`; not approved or merged. The data-only oracle covers both ledger views, § 11 assignment, splits, readiness, archive and blocked EXTF conventions. Seven register rows are `geprüft`; six remain `verify-before-production`. Blocks M7 export and Page 07 tax KPIs until approval. |
...
- Page 03 has a complete approved and merged transcription. Page 04 has a complete prepared
  transcription and exact 16-ID data oracle on `slice/docs-11`, pending Emir's approval. Pages 06
  and 07 have no complete transcription.
...
| D2 | **In progress — `docs/11` prepared for review** | Create source-backed `docs/09`–`docs/16` | `docs/09`, approved `docs/15`, `docs/12`, `docs/16` and `docs/10` are merged. `docs/11` and its exact 16-ID data-only oracle are prepared but unapproved and uncommitted. No tax-export implementation exists. |
...
- Make TAX_ADVISOR read-only outside the future adviser-profile and account-mapping write exception
  prepared in `docs/11`; no other write is permitted.
...
`docs/11` is prepared but unapproved and uncommitted. Implementation remains paused; this slice,
later D2 and D3 gates still require Emir's review.
After all three
