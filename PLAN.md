...
- **M10-F technically complete and review-clean locally (17.09.2026).** HEAD is `b93dc24` on
  ...
  edges. The final UI review is clean after the reminder-overlap and sensitivity-semantics fixes,
  and the boundary audit is clean. The ordinary statement fingerprint remains
  `c4eecb355d57cec620dfcb0134fc9141` / `149275` bytes. True browser 200% zoom and DB-backed browser
  proof remain coverage limits. Nothing was committed, merged or pushed; all production blockers
  remain active.
- **M10-I4 technically complete and locally merged into `main` (`97f6448`, 16.09.2026).**
  ...
  The slice branch is pushed; `main` was fast-forwarded locally with Emir's authorization and was
  not pushed. The pre-merge full gate is green. M10-F programme closure is recorded below; all
  Page-07 production blockers remain active.
...
- **M10-I2 technically complete and verified locally (16.09.2026).** Migration `0045` adds exactly
  ...
  is now locally merged; every Page-07 production blocker remains active. Only the I4 slice branch
  was pushed, not `main`.
...
| Page 07 — Investment-KPIs   | `docs/14-investment-kpis.md`                                    | `14-F01…F14`, with an exact `KPI-*` alias map             | Complete transcription approved and merged 21.08.2026 with all 14 data-only fixtures, 18 register rows and explicit Page-03/Page-09/Page-11 boundaries. M10-I0–I4 ship executable fixtures, the pure engine, immutable persistence, owner API, cockpit and frozen Bank-PDF. M10-F closure is recorded below; flagged conventions, interest-source ambiguity and absent concept sources still block production. |
...
| M10   | **Technically complete and review-clean locally; closure changes uncommitted**                                                                                       | Portals and investment                               | R0–R4 ship renter activation, isolated context, overview, immutable publication, source-derived display metadata and portal screens. I0–I4 ship the executable Page-07 fixtures, pure engine, immutable persistence, D4 entitlement, owner API, cockpit and deterministic Bank-PDF. M10-F adds migration `0047`, final boundary/UI review and green fast/full/demo gates. True browser 200% zoom and DB-backed browser proof remain coverage limits; all production blockers remain active.                                                                                                                                                                                                                                                                                                                                                                                  |
...
M10-R3 and migration `0043` were completed later; their verification is recorded below.
...
window is closed. The full gate passes with `2146` Python and `258` web tests. M10-I0/I1 is
committed as `58f8f36` and locally merged; nothing was pushed.
...
source is committed as `e9a17e3` and locally merged into `main`; nothing was pushed and all Page-07
production blockers remain active.

##### M10-I3 — investment API and entitlement

**Agents:** `spec-scribe`, then `engine-implementer` for migration/model/rules-store work, then
`app-implementer` for the API; `boundary-auditor` required.

- RED contract: `docs/14` § 4.8 and fixtures `M10-INV-F01…F10` fix migration `0046`, the exact
  route surface and atomic behavior before implementation.
- Migration `0046` adds one immutable account-scoped `investment_entitlement_event` stream with
  account-safe Membership/predecessor FKs, immediate-version/no-fork enforcement, forced RLS and
  rejected rewrites. Database RLS owns account scope; API authorization separately requires the
  authorizing membership to be `OWNER`.
- Exact routes under `/a/{account_id}/investment`: GET/POST entitlement, POST cases, and stored KPI,
  sensitivity and Bank-view GETs for one `case_key`. No case list, correction or PDF route.
- `EMPLOYEE`, `TAX_ADVISOR`, foreign Memberships and renter contexts receive no investment access.
  An absent or latest-disabled D4 entitlement denies every case route server-side.
- Case create accepts only normalized facts plus optional layout version, resolves the versioned
  server-owned Page-07 rule bundle, runs the pure engine and atomically appends input v1 plus one
  result. Invalid input stores nothing; a valid engine hard block is persisted.
- Reads return stored snapshots only and never recalculate. Foreign or missing `case_key` is `404`;
  renter identity, pricing, plan names, purchase flow and billing integration remain absent.

**Technical completion (16.09.2026):** the approved `docs/14` § 4.8 contract and fixtures
`M10-INV-F01…F10` are implemented through migration `0046`, the versioned server-owned rule bundle
...
I4 is committed as `97f6448`, its slice branch is pushed, and it is locally merged into `main`.
`main` was not pushed. UI proof is fixture-based, not database-backed or true browser
200% zoom. Direct renter-only PDF identity and changed-canonical default-successor fixtures remain
coverage limits; their source guards were audited. M10-F closure is recorded below; production
approval remains blocked.

#### M10-F — review and closure

**Technical closure — 17.09.2026:** migration `0047` hardens activation server time/expiry,
publication payloads/successor uniqueness and entitlement actor/chronology. Its final foreign-account
guard defers refusal to forced RLS without weakening the existing test; `26` focused DB checks pass.
The final fast, full and non-fresh demo gates pass `2335` Python and `271` web tests, with `81` RLS
tables and `163` FK edges. An empty disposable database migrated `0040 → 0047`; development and fresh schema
definitions/privileges match after order normalization at SHA-256
`87d970af15b49e4c5a6062050123787f549f60d4fc602c163005c98b7dcdb115`.

The final UI review is clean after the reminder-overlap and sensitivity-semantics fixes. The final
boundary audit is clean. The ordinary statement fingerprint is unchanged at
`c4eecb355d57cec620dfcb0134fc9141` / `149275` bytes. Browser diagnostics use mocked responses and
CSS scaling/equivalent viewports; true browser 200% zoom and DB-backed browser cases remain coverage
limits. Nothing was committed, merged or pushed. Technical closure clears no production blocker.
...
M6-B ships finalized owner-only tenant archives. M10-R3/R4 add isolated renter portal publication
and screens; scheduled/email delivery remains M9 work and its production prerequisites remain open.
