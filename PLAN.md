...
- M5 is complete and merged into `main` as `a748729`: secure bootstrap, backend
  ...
  M6-C and with it M6.**
- **G1 is technically complete and locally merged (`6c257f8`).** M9 extends that foundation on the
  uncommitted `slice/m9-guards-delivery`: all W1–W8 projections and `12-F01`–`12-F24`, migration
  `0025`, account-scoped jobs/API, immutable delivery evidence and `/waechter` are prepared.
  `main` is at `3d69013` and `origin/main` matches it: M7-E and everything before it are
  pushed to GitHub. Only M9 is unpushed, because it is uncommitted on its own branch.
  ...
  M7-F and M8 are parked. M9 is the active prepared milestone; M10 renter publication remains open.
...
| M8 | **Parked** | Document and letter engine | Approved routing exists, but missing clause/letter catalogues still block implementation. |
| M9 | **Demo gate green and boundary review clean; final statement review red 25.08.2026; uncommitted and unmerged** | Reminders, email and checklists | W1–W8, immutable migration `0025`, account-scoped jobs/API, default-off delivery and `/waechter` are implemented. The M9-R endpoint, membership, trigger and writer-trust defects are repaired and the fresh boundary review is clean. `scripts/gate.sh demo` is green (`1844` pytest, `130` vitest, `mypy --strict` over `243` files, 65 forced-RLS tenant tables, 130 account-scoped foreign keys) and the ordinary statement fingerprint is unchanged at `88eb8434eda65f8d7ff82826fc837a58` / `149269` bytes. The final statement review remains red because lower-case internal blocker keys can leak on `/waechter`, while the broad code heuristic hides useful `SHA-256` wording and gives some technical blockers a false legal fallback. Production delivery remains blocked by source/legal flags, absent real providers/workers, absent immutable UVI PDF bytes and an empty production checklist catalogue. |
...
**Status — 25.08.2026: demo gate green and boundary review clean; final statement review red;
uncommitted and unmerged on `slice/m9-guards-delivery`; M7-F and M8 remain parked.** Closure
verification found and fixed the following defects in the prepared slice:
