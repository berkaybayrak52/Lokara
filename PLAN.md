...
| 2 | **The Eigentümer line always exists** — even at 0,00 € | **Promoted, because row 1 does not reach a fully-let building without it.** `docs/03` § 9.2 convention 2: with no vacancy and no self-use there is no landlord party, so there is no owner bucket to hold the `Verteilungsrest` — and the engine must never hand it to a renter to force a block to reconcile. Those blocks therefore still use largest-remainder, which is **most buildings**. Berkay's Seite 01 D12 names the fix: an unconditional Eigentümer row. Touches `docs/02`, `docs/08` and the PDF, so it could not ride inside row 1. Until it lands, R1/R5/K9 governs only buildings with a vacancy. | medium |
...
| 4 | **M5 remainder** — roles in the API, portals, URL-carried context, switcher | Everything tenant-facing sits behind it. `slice/m5-pre-context-read` already carries **25 red fixtures**, plus the ENVIRONMENT guard and the `gate.sh` skip-hole fix. | medium |
...
| 9 | **UVI** — unterjährige Verbrauchsinformation | **Legally mandatory monthly** (§ 6a HeizkostenV); a missing one costs **3 %**. Needs row 4 for the tenant surface, monthly readings, and **Berkay's K13 answer on where average-user comparison values come from** — that input is not in hand. | medium |
| 10 | **M7** tax export + AfA (pages 03 + 04) | Specced, **not production-ready**. See the caveat below and in the M7 milestone. | high |
| 11 | **M8** clause engine · **M10 remainder** (tickets, activation codes, investment cockpit) · **mobile skeleton** | Broadest surface, furthest from the legal core. | high |

**Row 10's caveat, stated here and repeated in M7 because it is the expensive one to forget:**
