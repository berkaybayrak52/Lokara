...
Integer cents plus `decimal.Decimal`. Never `float`. `sum(shares) == input_total` is asserted,
not hoped for. `mypy --strict` clean.

Rounding is **per engine** — do not "correct" one to the other (`CLAUDE.md` DoD 4):
- **NK** — largest-remainder, `distribute_cents`.
