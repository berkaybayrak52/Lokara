...
- **`PLAN.md` milestone status** vs what exists. A milestone marked ✅ whose DoD you cannot
  ...
  built.
- **Stale migration banners.** `CLAUDE.md`, `README.md`, `lokara-arch.md` and `PLAN.md` carried
  "⚠️ stack migration in progress" and "the scaffold may still be the older TS code" notices; these
  were retired at Phase H. Any that reappear, or any new claim that a second backend exists, is a
  finding — it sends a fresh agent looking for code that is gone.
