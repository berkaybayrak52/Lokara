...
download. Paused M9 work integrated on `development` adds default-off scheduling/email, but remains
unverified and refuses UVI dispatch because no immutable PDF bytes exist and all UVI authority
blockers still apply. Portal publication remains M10.
...
15. **K12/DWD dependency.** Approved `docs/16` resolves the annual importer boundary to
    ...
    factors. U3 implements the distinct monthly DWD normalization adapters; paused M9 scheduling
...
| H8 annual comparison | **Shipped for normalized annual factors and raw/no-prior fallbacks** | U1–U5 implement the monthly UVI path; paused, unverified M9 scheduling/email on `development` remains default-off and blocked for missing immutable UVI PDF bytes; portal publication is M10 |
...
owner-downloadable renter document. Paused M9 work on `development` provides the scheduling/email
boundary but refuses UVI dispatch until immutable PDF bytes and production authority exist; M10
owns renter publication.
