...
- **Retention vs deletion = "two-clocks" model:**
  - **Uhr A** — renter *portal access* ends after the §556 window (final NK statement + buffer).
  - **Uhr B** — actual *deletion*, run separately **per data class**:
...
- Consistent disclaimer on every legal/tax output: *"Lokara ist ein Werkzeug, keine Rechts- oder
  Steuerberatung."* Keep the StBerG/legal-advice line out of the product.
