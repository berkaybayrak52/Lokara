...
date guards, and `docs/13` owns future clause selection, warnings and routing. `docs/14` consumes
only user-entered planning amounts for Page-02-owned non-allocable category identities; it neither
reclassifies catalogue rows nor turns those scenario amounts into ledger/tax truth. Page 06 does not
