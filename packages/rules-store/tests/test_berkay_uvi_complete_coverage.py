...
def test_block_c_uses_monthly_degree_days_and_preserves_raw_fallback() -> None:
    ...
    assert block_c["rounding_note"] == "source_example_percent_uses_rounded_display_values"
...
def test_all_nine_annual_dwd_fixtures_and_import_guards_are_preserved() -> None:
    ...
    assert DWD_ANNUAL_IMPORT_CONTRACT["monthly_degree_day_dataset"] is None
    assert DWD_ANNUAL_IMPORT_CONTRACT["station_to_plz_mapping"] is None
    assert DWD_ANNUAL_IMPORT_CONTRACT["monthly_uvi_status"] == "verify-before-production"
